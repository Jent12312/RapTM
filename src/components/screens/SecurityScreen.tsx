...
export default function SecurityScreen({ onClose }: Props) {
  ...
  const handleChangePasscode = async () => {
    if (newPasscode.length < 4) return addToast(t(language, 'error'), 'error');
    if (newPasscode !== confirmPasscode) return addToast(t(language, 'error'), 'error');
      ...
      if (!isTwoFactorEnabled) {
        if (!token) {
          ...
          if (res.ok) {
            ...
            addToast(data.error || t(language, 'sec2faSetupError'), 'error');
          }
        } else {
          ...
          if (res.ok) {
            ...
            const data = await res.json();
            addToast(data.error || t(language, 'secInvalidCode'), 'error');
          }
        }
      } else {
        // Disable Flow
  ...
  const handleUpdatePhone = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        addToast(t(language, 'secSaveError'), 'error');
      }
    } catch (e) {
      addToast(t(language, 'requestError'), 'error');
    } finally {
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div className="text-left">
          ...
          <p className="text-[10px] font-bold text-slate-400 mt-0.5">{value || t(language, 'unverified')}</p>
        </div>
      </div>
      <div className="flex items-center gap-2">
        {isVerified ? (
          <div className="bg-emerald-50 text-emerald-600 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase ring-1 ring-emerald-100">{t(language, 'referralStatusActive')}</div>
        ) : (
          <div className="bg-amber-50 text-amber-600 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase ring-1 ring-amber-100">{t(language, 'edit')}</div>
        )}
      </div>
    </button>
  );
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-50 flex flex-col animate-in slide-in-from-right duration-300">
      ...
      <div className="bg-white px-5 py-4 flex items-center gap-3 border-b border-slate-100 shadow-sm sticky top-0 z-10">
        ...
        <h2 className="text-lg font-black text-slate-800 tracking-tight">{t(language, 'security')}</h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 pb-20">
        ...
        <div className="bg-slate-900 rounded-[2.5rem] p-6 text-white relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-4">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-[10px] font-black opacity-40 uppercase tracking-[0.2em] mb-1">{t(language, 'secProtectionLevel')}</p>
            ...
            <p className="text-[10px] font-bold text-slate-400">
              {securityScore < 100 ? t(language, 'secLowProtection') : t(language, 'secFullProtection')}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <MenuItem 
            ...
            value={user?.phone ? `+${user.phone}` : t(language, 'secPhoneLinkDesc')} 
          ...
          <MenuItem 
            ...
            value={isTwoFactorEnabled ? t(language, 'sec2faDesc') : t(language, 'sec2faNotSet')} 
          ...
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-500">
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-black text-slate-800">{t(language, 'secChangePin')}</h3>
              </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 ml-2 uppercase tracking-widest">{t(language, 'secNewPin')}</label>
                <input 
                  type="password" 
                  inputMode="numeric"
                  value={newPasscode}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 ml-2 uppercase tracking-widest">{t(language, 'secConfirmPin')}</label>
                <input 
                  type="password" 
                  inputMode="numeric"
              ...
              >
                {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : t(language, 'secSavePin')}
              </button>
            </div>
          </div>
        </div>
           ...
           <div className="p-8 space-y-8 max-w-md mx-auto">
              <div className="flex justify-between items-center">
                 <h3 className="text-2xl font-black text-slate-800 tracking-tight">
                   {twoFactorData?.action === 'disable' ? t(language, 'sec2faDisable') : t(language, 'sec2faEnable')}
                 </h3>
              ...
              <div className="space-y-6 text-center">
                 ...
                 <div className="space-y-2">
                    <p className="text-sm font-bold text-slate-800">
                      {twoFactorData?.action === 'disable' ? t(language, 'sec2faDisableInput') : t(language, 'sec2faEnableScan')}
                    </p>
                    {twoFactorData?.secret && (
                      <>
                        <p className="text-xs text-slate-400">{t(language, 'sec2faManual')}</p>
                 ...
                 <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t(language, 'secEnter6Digit')}</label>
                    <input 
                      type="text" 
                      inputMode="numeric"
                      maxLength={6}
                 ...
                 <div className={`${twoFactorData?.action === 'disable' ? 'bg-rose-50 border-rose-100' : 'bg-emerald-50 border-emerald-100'} p-6 rounded-[2rem] border flex gap-4 text-left`}>
                    ...
                    <p className={`text-[11px] ${twoFactorData?.action === 'disable' ? 'text-rose-800' : 'text-emerald-800'} font-bold leading-relaxed`}>
                       {twoFactorData?.action === 'disable' 
                         ? t(language, 'sec2faDisableWarning') 
                         : t(language, 'sec2faEnableSuccess')}
                    </p>
                 </div>
                 ...
                 >
                   {isLoading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : t(language, 'secConfirm')}
                 </button>
              </div>
           </div>
        </div>
          ...
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-500">
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-black text-slate-800">{t(language, 'secLinkPhone')}</h3>
              </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 ml-2 uppercase tracking-widest">{t(language, 'secPhoneNumber')}</label>
                <input 
                  type="tel" 
                  value={phone}
              ...
              <div className="bg-amber-50 p-4 rounded-2xl flex gap-3">
                 ...
                 <p className="text-[10px] text-amber-800 font-bold">{t(language, 'secPhoneWarning')}</p>
              </div>
              ...
              <button 
                ...
                className="w-full py-5 bg-slate-900 text-white rounded-[1.5rem] font-black text-xs tracking-[0.2em] shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-3"
              >
                {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : t(language, 'secSavePhone')}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-500">
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-black text-slate-800">{t(language, 'secLinkEmail')}</h3>
              </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 ml-2 uppercase tracking-widest">{t(language, 'secEmailAddress')}</label>
                <input 
                  type="email" 
                  value={email}
              ...
              <div className="bg-blue-50 p-4 rounded-2xl flex gap-3">
                 ...
                 <p className="text-[10px] text-blue-800 font-bold">{t(language, 'secEmailDesc')}</p>
              </div>
              ...
              <button 
                ...
                className="w-full py-5 bg-slate-900 text-white rounded-[1.5rem] font-black text-xs tracking-[0.2em] shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-3"
              >
                {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : t(language, 'secSaveEmail')}
              </button>
            </div>
          </div>
        </div>
